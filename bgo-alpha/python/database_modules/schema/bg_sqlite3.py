...
-- INTEGER PRIMARY KEY and BLOB to implement autoincrementing types 
...
CREATE INDEX module_class_filename_idx ON module_class(filename);
CREATE INDEX module_class_mod_idx ON module_class(module);
...
CREATE INDEX file_class_idx ON file(class);
CREATE INDEX file_image_idx ON file(image);
...
CREATE TABLE patch (
	...
	file_offset	INTEGER,		-- offset in file to apply patch
	length		integer,	-- number of bytes to patch
	bytes		BLOB		-- bytes to patch with
);
...
CREATE TABLE process_file_map (
	...
	process		integer REFERENCES process_image(id),
	file		integer REFERENCES file(id),
	address		INTEGER		-- load address of file
);
...
create table section (
	...
	load_addr	INTEGER,
	size		INTEGER,
	alloc_size	INTEGER,				-- in-memory size
	arch		integer REFERENCES arch(id)		DEFAULT 1,
...
CREATE INDEX section_arch_idx ON section(arch);
CREATE INDEX section_compiler_idx ON section(compiler);
CREATE INDEX section_source_idx ON section(source);
...
CREATE TABLE process_section_map (
	...
	address		INTEGER			-- load address of section
);
...
--CREATE INDEX datatype_name_idx ON datatype(name);
CREATE INDEX datatype_basetype_idx ON datatype(basetype);
CREATE INDEX datatype_reference_idx ON datatype(reference);
CREATE INDEX datatype_container_idx ON datatype(container);

-- DATA : A data address. Has an associated data type. 
CREATE TABLE data (
	id		INTEGER PRIMARY KEY,
	file_offset	INTEGER,	-- offset (from file start)
	address		INTEGER,	-- load address
...
CREATE INDEX data_datatype_idx ON data(datatype);
...
CREATE TABLE insn_def (
	...
	bytes		BLOB,		-- bytes in instruction
	signature	BLOB,
	size		integer,
...
CREATE INDEX insn_def_class_idx ON insn_def(class);
...
CREATE INDEX op_bit_reg_idx ON op_bit(reg);
...
CREATE TABLE op_expr (
	...
	scale		integer,
	disp		INTEGER				-- displacement
);

CREATE INDEX op_expr_base_idx ON op_expr(base);
CREATE INDEX op_expr_index_idx ON op_expr(idx);
...
CREATE INDEX insn_op_def_map_dtype_idx ON insn_op_def_map(datatype);
...
CREATE TABLE user_comment (
	id		INTEGER PRIMARY KEY,		-- ID from sequence
